-- Prompt 09C: integrates ACTIVE loan installments into the existing
-- Prompt 07 Payment Engine (section 2 — "Loan repayment MUST fit into
-- that exact pipeline. Do NOT create a second payment system.").
--
-- Every RPC below keeps its exact existing signature (CREATE OR
-- REPLACE) and every existing behavior for contribution-only payments
-- is byte-for-byte unchanged — the only difference is that the
-- allocation walk now also considers a borrower's ACTIVE loan
-- installments, via payment_compute_combined_allocation_plan
-- (20260904092000) instead of contribution_compute_payment_allocation_plan
-- alone. One payment/one wallet allocation still creates at most one
-- cashbook movement, exactly as before — loan allocations never add a
-- second cashbook entry (section 3/26).

-- ---------------------------------------------------------------------
-- rpc_preview_payment_allocation — allocation lines now carry
-- obligation_kind ('CONTRIBUTION' | 'LOAN_INTEREST' | 'LOAN_PRINCIPAL')
-- plus loan context (loan_account_id/loan_number/installment_number)
-- when applicable, alongside the existing contribution context.
-- ---------------------------------------------------------------------
...
begin
  ...
  from public.payment_compute_combined_allocation_plan(p_group_id, p_membership_id, p_amount) plan;
...
-- ---------------------------------------------------------------------
-- rpc_post_payment — the combined plan now posts CONTRIBUTION_COMPONENT
-- rows (unchanged, with their snapshot columns) alongside LOAN_PRINCIPAL/
-- LOAN_INTEREST rows. After posting, every distinct loan touched by this
-- payment is rechecked for closure (section 19) — never merely "payment
-- amount reached", always the server-authoritative outstanding
-- recomputation in loan_account_recheck_closure. Still exactly one
-- cashbook INFLOW for the FULL payment amount (invariant B, unchanged).
-- ---------------------------------------------------------------------
...
begin
  ...
  for v_plan in
    select * from public.payment_compute_combined_allocation_plan(p_group_id, p_membership_id, p_amount)
  loop
...
-- ---------------------------------------------------------------------
-- rpc_reverse_payment — reversing a payment that included loan
-- allocations must restore both loan components' outstanding (section
-- 17) and, if the loan had since become CLOSED off the back of this
-- payment, reopen it (section 20). The reversal mechanism itself is
-- completely unchanged: payment_allocations rows are NEVER edited or
-- deleted (their "active" status already derives purely from
-- payments.status = 'POSTED', which flips to REVERSED here) — reopening
-- a loan is a pure side effect of that same derivation.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- rpc_preview_wallet_allocation — same combined-plan context as the
-- payment preview.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- rpc_allocate_member_wallet — wallet-to-loan settlement (section 10):
-- zero cashbook movement (unchanged — this function never called
-- financial_account_post_entry and still never does), creates loan
-- allocation records exactly like the payment path, reduces wallet
-- balance and loan outstanding, and recognizes interest income only
-- via the same universal derivation every other read model uses (no
-- special-casing wallet vs payment source). Also rechecks closure for
-- every loan touched.
-- ---------------------------------------------------------------------
...
begin
  ...
  from public.payment_compute_combined_allocation_plan(p_group_id, p_membership_id, p_amount);
